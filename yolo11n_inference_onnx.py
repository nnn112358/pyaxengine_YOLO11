...
def preprocess(image_path, input_size):
    ...
    戻り値:
    input_tensor: 前処理された入力テンソル
    original_shape: 元の画像サイズ
    image: RGB形式の画像データ
    """
    image = cv2.imread(image_path)
    if image is None:
        raise FileNotFoundError(f"画像ファイルを読み込めません: {image_path}")
    ...
    input_tensor = np.expand_dims(
        resized_image, axis=0).astype(np.float32) / 255.0
...
def main():
    ...
    INPUT_SIZE = (input_shape[3], input_shape[2])  # NCHW形式なのでW,Hの順
    ...
    cv2.imwrite(OUT_IMAGE_PATH, cv2.cvtColor(
        original_image, cv2.COLOR_RGB2BGR))

    # aarch64以外の場合のみ画像表示を試行
    if platform.machine() != 'aarch64':
        try:
            cv2.imshow('Detection Result', cv2.cvtColor(
                cv2.resize(original_image, (640, 480)), cv2.COLOR_RGB2BGR))
            cv2.waitKey(0)
            cv2.destroyAllWindows()
        except Exception as e:
            print(f"Warning: Could not display the image window: {e}")
            print("The result has been saved to", OUT_IMAGE_PATH)
